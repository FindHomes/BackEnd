...
//    }
    private String createGPTCommand(String userInput, String keywords, String publicData) {
        return String.format(
                ...
                "이 가중치는 해당 시설이 사용자 요구와 얼마나 관련이 있는지를 나타냅니다. 예를 들어 '음식점_버거킹-0.3'는 " +
                "사용자가 버거킹과 가까운 집을 원할 경우, 음식점 중에서 버거킹의 중요도가 0.3임을 의미합니다. " +
                "'음식점_버거킹'과 같은 형식에서 '음식점'은 큰 범주를, '버거킹'은 그 범주 안의 특정 키워드를 의미합니다. " +
                "이처럼 포함관계의 속하는 데이터는 '_'로 표시합니다"+
                "이 섹션과 3,4번 섹션에서 제공하는 모든 가중치의 총합은 반드시 1이어야 합니다. " +
                "예시: '음식점_버거킹-0.3, 피시방-0.2, 미용실-0.1, 병원-0.4'.\n\n" +
                "3. **공공 데이터**: 공공 데이터와 관련된 항목을 명시하고, 각 항목의 중요도를 가중치로 표시합니다. " +
                "이 가중치는 해당 공공 데이터가 사용자 요구와 얼마나 관련이 있는지를 나타냅니다. 예를 들어 '범죄율-0.3'는 " +
                "사용자가 안전한 지역을 원할 경우, 범죄율이 0.3의 중요도를 가지는 것을 의미합니다. " +
                "이 섹션과 이전 섹션에서 제공하는 모든 가중치의 총합은 반드시 1이어야 합니다. " +
                "즉 2번 섹션과 3번 섹션과 4번 섹션의 가중치 총합이 1이어야합니다."+
                "예시: '교통사고율-0.2, 화재율-0.1, 범죄율-0.3, 생활안전-0.2, 자살율-0.2'.\n\n" +
                "4. 특정 좌표: 사용자가 가까웠으면 하는 특정 지점의 좌표입니다. "+
                "예를 들어 '네이버 본사와 강남역이랑 가까웠으면 좋겠다'와 같은 조건을 받으면 네이버본사_(37.359512,127.105220)-0.2, 강남역_(37.497940,127.027620)-0.2 와 같이 나타냅니다."+
                "모든 섹션의 형식을 정확히 준수하여, 불필요한 텍스트 없이 응답해주세요. 반환 형식 예시는 다음과 같습니다."+
                "'관리비-20이하, 층수-3층, 복층-없음/음식점_버거킹-0.2, 피시방-0.2, 미용실-0.1, 병원-0.1" +
                "/교통사고율-0.1, 화재율-0.1, 범죄율-0.1/강남역_(37.497940,127.027620)-0.1'."
                , userInput, keywords, publicData
        );
    }
